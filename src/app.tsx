...
export function App(): ReactElement {
  ...
  return <ConfigProvider theme={{ token: { colorPrimary: '#2F6BFF', colorBgLayout: '#F5F7FA', borderRadius: 8, fontFamily: "'Microsoft YaHei', sans-serif" } }}><AntApp><Layout className="app-frame">
    ...
    <Layout><Layout.Sider className="app-sidebar" width={232} theme="dark"><div className="sidebar-inner"><div className="sidebar-caption">WORKSPACE / 2026</div><Menu theme="dark" mode="inline" selectedKeys={[activeNav]} items={NAV_ITEMS} onClick={({ key }) => setActiveNav(key as NavKey)} /><div className="sidebar-footer"><span>桌面版</span><small>v{version}</small></div></div></Layout.Sider><Layout className="main-layout"><div className="page-heading"><div><Typography.Text className="eyebrow">毕业设计项目工作台</Typography.Text><Typography.Title level={1}>{NAV_ITEMS.find((item) => item.key === activeNav)?.label}</Typography.Title></div>{activeNav !== 'development' && <Button type="primary" icon={<AppstoreOutlined />} onClick={() => setActiveNav('projects')}>新建项目</Button>}</div><Layout.Content className="app-content">{activeNav === 'dashboard' && <ProjectStatisticsPage onNavigateToProjects={(next = 'all') => { setStatus(next); setActiveNav('projects'); }} onEmptyAction={handleEmptyAction} />}{activeNav === 'projects' && <ProjectManagementPage initialStatus={status} onImportTemplate={() => setActiveNav('projects')} onDevelop={() => setActiveNav('development')} />}{activeNav === 'development' && <ProjectDevelopmentPage />}</Layout.Content></Layout></Layout>
