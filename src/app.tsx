...
import { ArrowLeftOutlined, BarChartOutlined, CodeOutlined, CloseOutlined, DatabaseOutlined, DollarOutlined, MinusOutlined, ProjectOutlined, RobotOutlined, ArrowsAltOutlined, SettingOutlined } from '@ant-design/icons';
...
type NavKey = 'dashboard' | 'projects' | 'development' | 'orders' | 'settings';
type SettingsKey = 'ai' | 'wechat';
const NAV_ITEMS = [{ key: 'dashboard', icon: <BarChartOutlined />, label: '仪表盘' }, { key: 'projects', icon: <ProjectOutlined />, label: '项目管理' }, { key: 'development', icon: <CodeOutlined />, label: '项目开发' }, { key: 'orders', icon: <DollarOutlined />, label: '成单分析' }, { key: 'settings', icon: <SettingOutlined />, label: '设置' }];
...
export function App(): ReactElement {
  const [activeNav, setActiveNav] = useState<NavKey>('dashboard'); const [visitedNav, setVisitedNav] = useState<Set<NavKey>>(() => new Set(['dashboard'])); const [sourceNav, setSourceNav] = useState<NavKey>('dashboard'); const [settingsKey, setSettingsKey] = useState<SettingsKey>('ai'); const [status, setStatus] = useState<ProjectStatusFilter>('all'); const [maximized, setMaximized] = useState(false);
