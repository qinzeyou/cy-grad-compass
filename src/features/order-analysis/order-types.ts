...
export interface DealCandidate { id: string; sessionId: string; sessionName: string; customerName: string; projectName: string; confidence: number; amount: number | null; dealTime: number | null; evidence: WechatMessage[]; matchedFolder: ProjectFolder | null; status: 'candidate' | 'confirmed' | 'ignored'; }
...
export interface OrderRecord { id: string; customerName: string; sessionId: string; projectName: string; folderPath: string | null; confirmedAt: number; transactions: Transaction[]; maintenance: MaintenanceRecord[]; evidence: WechatMessage[]; }
