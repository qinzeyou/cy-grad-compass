...
export function OrderAnalysisPage(): ReactElement {
  ...
  return <div className="order-analysis-page">
    ...
    <Card title="订单" className="order-table-card"><Table rowKey="id" loading={loading} dataSource={orders} columns={columns} pagination={{ pageSize: 8 }} scroll={{ y: '100%' }} onRow={(row) => ({ onClick: () => { setSelected(null); setSelectedOrder(row); } })} /></Card>
    <Modal title={selected ? `${contactName(selected)} · 成单线索` : '成单线索'} open={selected !== null && !confirmOpen} width={680} centered onCancel={() => setSelected(null)} footer={null} className="candidate-detail-modal">
      {selected && <article className="candidate-detail-shell">
        <header className="candidate-detail-header">
          ...
          <Space className="candidate-modal-actions" wrap><Button onClick={() => markNotDeal(selected)}>未成单</Button><Button danger icon={<DeleteOutlined />} onClick={() => removeCandidate(selected)}>删除</Button><Button type="primary" icon={<CheckCircleFilled />} onClick={() => openConfirm(selected)}>确认成单</Button></Space>
        </header>
