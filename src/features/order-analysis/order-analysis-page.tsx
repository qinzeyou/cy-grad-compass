...
import { Alert, App as AntdApp, Avatar, Button, Card, Collapse, DatePicker, Empty, Form, Input, InputNumber, Modal, Select, Space, Spin, Table, Tag, Typography } from 'antd';
...
export function OrderAnalysisPage(): ReactElement {
  ...
  const openConfirm = (candidate: DealCandidate) => { setSelected(candidate); form.setFieldsValue({ projectName: candidate.projectName, customerName: candidate.customerName, amount: candidate.amount ?? undefined, folderMode: 'new', folderPath: undefined }); void window.desktopApi.listOrderProjectFolders().then(setFolders); setConfirmOpen(true); };
  ...
  return <div className="order-analysis-page">
    ...
    <Card title="订单台账" className="order-table-card"><Table rowKey="id" loading={loading} dataSource={orders} columns={columns} pagination={{ pageSize: 8 }} scroll={{ y: '100%' }} onRow={(row) => ({ onClick: () => { setSelected(null); setSelectedOrder(row); } })} /></Card>
    <Modal title={selected ? `${contactName(selected)} · 成单线索` : '成单线索'} open={selected !== null && !confirmOpen} width={680} onCancel={() => setSelected(null)} footer={selected ? <Space><Button onClick={() => markNotDeal(selected)}>确认未成单</Button><Button danger onClick={() => removeCandidate(selected)}>删除</Button><Button type="primary" onClick={() => openConfirm(selected)}>确认成单</Button></Space> : null}>
      {selected && <Space direction="vertical" size={16} style={{ width: '100%' }}><div className="candidate-profile"><Avatar size={52} src={selected.avatarUrl}>{contactName(selected).slice(0, 1)}</Avatar><div><Typography.Title level={4} style={{ margin: 0 }}>{contactName(selected)}</Typography.Title>{selected.nickname && selected.nickname !== contactName(selected) ? <Typography.Text type="secondary">昵称：{selected.nickname}</Typography.Text> : <Typography.Text type="secondary">微信名称未设置</Typography.Text>}</div></div><div><Typography.Text type="secondary">匹配项目</Typography.Text><Typography.Title level={4}>{selected.projectName}</Typography.Title><Typography.Text>预计金额：{selected.amount == null ? '待确认' : money(selected.amount)} · 成交时间：{date(selected.dealTime)}</Typography.Text></div><Typography.Paragraph>成交判断置信度：{Math.round(selected.confidence * 100)}%</Typography.Paragraph><Collapse ghost items={[{ key: 'evidence', label: `成单聊天记录（${selected.evidence.length} 条）`, children: <div className="evidence-timeline">{selected.evidence.map((item) => <div key={item.id}><Typography.Text type="secondary">{item.senderName}{item.isSelf ? '（我）' : ''} · {date(item.sentAt)}</Typography.Text><p>{item.text}</p></div>)}</div> }]} /></Space>}
    </Modal>
