...
import type { DevelopmentMode, DevelopmentSessionDetail, DevelopmentRunStatus } from './project-development-types';

type Props = { session: DevelopmentSessionDetail; runStatus: DevelopmentRunStatus; error: string; onSend: (message: string, mode?: DevelopmentMode) => void; onStart: () => void };
...
export function DevelopmentChatPanel({ session, runStatus, error, onSend, onStart }: Props): ReactElement {
  ...
  const [mode, setMode] = useState<DevelopmentMode>(session.phase === 'development' ? 'development' : 'discussion');
