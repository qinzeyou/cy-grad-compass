...
1. 用户进入“项目开发”；
2. 先在左侧选择一个项目管理中的已有项目；未选择项目时不加载会话、不显示对话输入框，也不能启动 AI；
3. 选择项目后，左侧只显示该项目的开发会话；用户可打开历史会话或新建会话；
4. 用户发送消息，与 AI 讨论需求；
5. 讨论阶段 Codex 只能读取项目，不能修改文件；
6. 用户点击“开始开发”；
7. 应用恢复同一 Codex thread，并允许在项目目录内写入；
8. 中间栏显示真实 AI 回复；
9. 右侧显示真实命令、文件变更、错误和运行耗时；
10. 用户可停止当前运行；
11. 运行结束后可继续发送补充要求，后续消息保持开发权限。
